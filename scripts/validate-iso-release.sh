...
iso_path="$repo_root/ISO/0.0.3/out/unexus-os-0.0.3-x86_64.iso"
default_report_dir="$repo_root/ISO/0.0.3/out/release-checks"
...
Options:
  ...
  --iso PATH       ISO image to validate (default: ISO/0.0.3/out/unexus-os-0.0.3-x86_64.iso)
...
under ISO/0.0.3/out/release-checks/.
EOF
}
...
static_checks() {
    ...
    sh -n "$repo_root/ISO/0.0.3/build-iso.sh"
    sh -n "$repo_root/scripts/test-iso-vm.sh"
    sh -n "$repo_root/scripts/install-os.sh"
    sh -n "$repo_root/ISO/0.0.3/profile/airootfs/root/customize_airootfs.sh"
    sh -n "$repo_root/ISO/0.0.3/profile/airootfs/usr/local/bin/unexus-live-boot-mode"
    sh -n "$repo_root/ISO/0.0.3/profile/airootfs/usr/local/bin/unexus-live-smoke-test"
}
...
build_image() {
    if [ "$(id -u)" -eq 0 ]; then
        sh "$repo_root/ISO/0.0.3/build-iso.sh"
        return
    fi

    if command -v sudo >/dev/null 2>&1; then
        sudo sh "$repo_root/ISO/0.0.3/build-iso.sh"
        return
    fi
